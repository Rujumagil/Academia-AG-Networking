(() => {
  ...
  const RELEASE = '20260820.49';
  const COURSE_ID = '7c4d9f60-8b0a-4b7b-9f2c-2d5e1a8c4f01';
  const MODULE_ID = '7c4d9f60-1002-4b7b-9f2c-2d5e1a8c4001';
  const PROMO_CODE = 'C2-PROMO';
  const MODULE_LABEL = 'Módulo 2';
  const SCROLL_KEY = 'academia-ag:utah-v2:m2-exam-scroll';
  let timer = null;
  ...
  function moduleDetailsNode() {
    ...
    return details.find(node => /^Módulo\s+2\b/i.test(node.querySelector('summary strong')?.textContent?.trim() || '')) || null;
  }
  ...
  function ensureSidebarStep() {
    ...
    if (!promo || !list || list.querySelector('.utah-v2-exam-row-m2')) return;

    const row = document.createElement('div');
    row.className = 'lesson-item utah-v2-exam-row utah-v2-exam-row-m2';
    row.innerHTML = `
      <span class="utah-v2-exam-dot" aria-hidden="true">?</span>
      <a href="#lesson/${COURSE_ID}/${escapeText(promo.id)}" data-utah-v2-m2-exam-link>
        <strong>Evaluación opcional</strong>
        <small>Repaso del Módulo 2 · 10 preguntas</small>
      </a>
      <small>Opcional</small>`;

    row.querySelector('[data-utah-v2-m2-exam-link]')?.addEventListener('click', () => {
  ...
  function markSidebarExam(bestPercentage = 0) {
    const row = moduleDetailsNode()?.querySelector('.utah-v2-exam-row-m2');
    if (!row) return;
    ...
    return `
      <div class="utah-v2-exam-top">
        <div>
          ...
          <h2>Salud, exámenes y preparación del vehículo</h2>
          <p class="utah-v2-exam-intro">Contesta este repaso para reforzar los puntos principales del módulo. No bloquea tu avance y puedes repetirlo las veces que quieras.</p>
        </div>
  ...
  function ensureExamCard() {
    ...
    card.setAttribute('aria-label', 'Evaluación opcional del Módulo 2');
  ...
  window.ACADEMIA_AG_UTAH_MODULE2_EXAM_V2 = { release: RELEASE, enhance };
  schedule();
})();
