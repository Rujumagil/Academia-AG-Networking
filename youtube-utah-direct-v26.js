(() => {
  ...
  function addStyles() {
    if (document.querySelector('#ag-youtube-direct-v26-style')) return;
    const style = document.createElement('style');
    style.id = 'ag-youtube-direct-v26-style';
    style.textContent = `
      .video-shell.ag-youtube-direct-v26{position:relative!important;width:100%!important;aspect-ratio:16/9!important;overflow:hidden!important;background:#05080d!important;border-radius:22px!important}
      .video-shell.ag-youtube-direct-v26>img,.video-shell.ag-youtube-direct-v26>.video-center,.video-shell.ag-youtube-direct-v26>.video-bar,.video-shell.ag-youtube-direct-v26>video,.video-shell.ag-youtube-direct-v26>.lesson-native-video,.video-shell.ag-youtube-direct-v26>.lesson-video-fallback{display:none!important}
      .video-shell.ag-youtube-direct-v26 iframe,.video-shell.ag-youtube-direct-v26 .ag-youtube-direct-holder{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;border:0!important;border-radius:inherit!important;background:#05080d!important}
  ...
  function clearShell(shell) {
    shell.classList.add('ag-youtube-direct-v26');
    shell.querySelectorAll('img,.video-center,.video-bar,video,.lesson-native-video,.lesson-video-fallback,#video-placeholder,.ag-utah-youtube-preload,.ag-youtube-direct-loading,.ag-youtube-direct-missing,.youtube-lesson-ended').forEach(n => n.remove());
    shell.querySelectorAll('iframe').forEach(frame => {
      if (!/youtube(?:-nocookie)?\.com/i.test(String(frame.src || ''))) frame.remove();
    });
  }
  ...
  function showLoading(shell, ctx) {
    ...
    box.className = 'ag-youtube-direct-loading';
    box.innerHTML = `<strong>Cargando video de YouTube…</strong><span>Lección ${ctx.lessonPosition} · Módulo ${ctx.modulePosition}</span>`;
    shell.appendChild(box);
  }

  function showMissing(shell, ctx) {
    clearShell(shell);
    const box = document.createElement('div');
    box.className = 'ag-youtube-direct-missing';
    box.innerHTML = `<strong>Video pendiente de asignar</strong><span>Esta lección todavía no tiene un enlace individual de YouTube cargado.</span>`;
    shell.appendChild(box);
  }
  ...
  async function mount(shell, ctx, videoId) {
    ...
    if (mountedKey === key && existing && /youtube(?:-nocookie)?\.com/i.test(String(existing.src || ''))) return;
    ...
    holder.id = `ag-youtube-direct-${String(ctx.lesson.id).replace(/[^a-z0-9_-]/gi,'')}`;
    ...
    player = new window.YT.Player(holder.id, {
      ...
      events: {
        ...
        onError(event) {
          mountedKey = '';
          shell.innerHTML = `<div class="ag-youtube-direct-missing"><strong>No pudimos cargar este video</strong><span>YouTube devolvió el código ${event.data}.</span></div>`;
        }
      }
    });
  }
  ...
  async function apply() {
    ...
    if (!videoId) {
      showMissing(shell, ctx);
      return;
    }

    if (!shell.querySelector('iframe') || mountedKey !== `${ctx.lesson.id}:${videoId}`) showLoading(shell, ctx);
    try {
      await mount(shell, ctx, videoId);
    } catch (error) {
      console.error('YouTube direct player error:', error);
      showMissing(shell, ctx);
    }
  }
  ...
  window.addEventListener('hashchange', () => { mountedKey = ''; schedule(); });
