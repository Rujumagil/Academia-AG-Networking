(() => {
  ...
  const RELEASE = '20260820.51';
  const COURSE_ID = '7c4d9f60-8b0a-4b7b-9f2c-2d5e1a8c4f01';
  const MODULE_ID = '7c4d9f60-1004-4b7b-9f2c-2d5e1a8c4001';
  const PROMO_CODE = 'C4-PROMO';
  const SCROLL_KEY = 'academia-ag:utah-v2:m4-exam-scroll';
  let timer = null;
  ...
  function moduleDetailsNode() {
    ...
    return details.find(node => /^Módulo\s+4\b/i.test(node.querySelector('summary strong')?.textContent?.trim() || '')) || null;
  }
  ...
  function ensureSidebarStep() {
    ...
    if (!promo || !list || list.querySelector('.utah-v2-exam-row-m4')) return;

    const row = document.createElement('div');
    row.className = 'lesson-item utah-v2-exam-row utah-v2-exam-row-m4';
    row.innerHTML = `
      <span class="utah-v2-exam-dot" aria-hidden="true">?</span>
      <a href="#lesson/${COURSE_ID}/${esc(promo.id)}" data-utah-v2-m4-exam-link>
        <strong>Evaluación opcional</strong>
        <small>Repaso del Módulo 4 · 10 preguntas</small>
      </a>
      <small>Opcional</small>`;

    row.querySelector('[data-utah-v2-m4-exam-link]')?.addEventListener('click', () => {
  ...
  function markSidebarExam(bestPercentage = 0) {
    const row = moduleDetailsNode()?.querySelector('.utah-v2-exam-row-m4');
    if (!row) return;
  ...
  function examMarkup(exam) {
    ...
    return `
      <div class="utah-v2-exam-top">
        <div>
          <span class="utah-v2-exam-kicker">Evaluación opcional · Módulo 4</span>
          <h2>Reglas del camino y señales</h2>
          <p class="utah-v2-exam-intro">Repasa Flex Lanes, freeway, Express/Carpool Lanes, Ramp Meters, marcas viales, velocidad, roundabouts, semáforos y derecho de paso. No bloquea tu avance y puedes repetirlo cuantas veces quieras.</p>
        </div>
  ...
  function ensureExamCard() {
    ...
    card.setAttribute('aria-label', 'Evaluación opcional del Módulo 4');
  ...
  window.ACADEMIA_AG_UTAH_MODULE4_EXAM_V2 = { release: RELEASE, enhance };
  schedule();
})();
