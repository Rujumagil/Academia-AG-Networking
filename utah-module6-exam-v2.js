(() => {
  ...
  const RELEASE = '20260821.53';
  const COURSE_ID = '7c4d9f60-8b0a-4b7b-9f2c-2d5e1a8c4f01';
  const MODULE_ID = '7c4d9f60-1006-4b7b-9f2c-2d5e1a8c4001';
  const FINAL_CODE = 'C6-25';
  const SCROLL_KEY = 'academia-ag:utah-v2:m6-exam-scroll';
  let timer = null;
  ...
  function moduleData() {
    return course()?.modules?.find(item => item.id === MODULE_ID) || null;
  }

  function finalLesson() {
    const module = moduleData();
    return module?.lessons?.find(item => item.lesson_code === FINAL_CODE)
      || [...(module?.lessons || [])].sort((a,b) => Number(b.position || 0) - Number(a.position || 0))[0]
      || null;
  }
  ...
  function moduleNode() {
    const c = course();
    if (!c) return null;
    const ordered = [...(c.modules || [])].sort((a,b) => Number(a.position || 0) - Number(b.position || 0));
    ...
    return details.find(node => /^Módulo\s+6\b/i.test(node.querySelector('summary strong')?.textContent?.trim() || '')) || null;
  }

  function ensureSidebar() {
    if (!isUtahRoute()) return;
    const lesson = finalLesson();
    const list = moduleNode()?.querySelector('.lesson-list');
    if (!lesson || !list || list.querySelector('.utah-v2-exam-row-m6')) return;

    const row = document.createElement('div');
    row.className = 'lesson-item utah-v2-exam-row utah-v2-exam-row-m6';
    row.innerHTML = `
      <span class="utah-v2-exam-dot" aria-hidden="true">?</span>
      <a href="#lesson/${COURSE_ID}/${esc(lesson.id)}" data-utah-v2-m6-exam-link>
        <strong>Evaluación opcional</strong>
        <small>Repaso del Módulo 6 · 10 preguntas</small>
      </a>
      <small>Opcional</small>`;

    row.querySelector('[data-utah-v2-m6-exam-link]')?.addEventListener('click', () => {
  ...
  function markSidebar(bestPercentage = 0) {
    const row = moduleNode()?.querySelector('.utah-v2-exam-row-m6');
    if (!row) return;
  ...
  function feedback(review = []) {
    ...
    if (!wrong.length) return '<div class="utah-v2-exam-perfect">✓ Todas tus respuestas fueron correctas en este intento.</div>';
    return `
      <section class="utah-v2-exam-feedback">
        <h3>Respuestas para reforzar</h3>
        <p>Revisa tu respuesta, la correcta y la explicación correspondiente.</p>
        <div class="utah-v2-exam-feedback-list">
          ${wrong.map(item => `
            <article class="utah-v2-exam-feedback-item">
              <strong>${Number(item.position || 0) ? `${Number(item.position)}. ` : ''}${esc(item.question || '')}</strong>
              <div class="utah-v2-exam-feedback-line"><span>Tu respuesta</span><span>${esc(String(item.selected_key || '').toUpperCase())}${item.selected_text ? ` · ${esc(item.selected_text)}` : ' · Sin respuesta'}</span></div>
              <div class="utah-v2-exam-feedback-line correct"><span>Respuesta correcta</span><span>${esc(String(item.correct_key || '').toUpperCase())}${item.correct_text ? ` · ${esc(item.correct_text)}` : ''}</span></div>
              ${item.explanation ? `<p class="utah-v2-exam-explanation"><strong>Para reforzar:</strong> ${esc(item.explanation)}</p>` : ''}
            </article>`).join('')}
        </div>
      </section>`;
  }
  ...
  function applyReview(form, review = []) {
    ...
    if (host) host.innerHTML = feedback(review);
  }

  function markup(exam) {
    const questions = exam?.questions || [];
    const best = Number(exam?.best_percentage || 0);
    return `
      <div class="utah-v2-exam-top">
        <div>
          <span class="utah-v2-exam-kicker">Evaluación opcional · Módulo 6</span>
          <h2>Emergencias, compartir el camino y tu récord</h2>
          <p class="utah-v2-exam-intro">Repasa choques, seguro, sistema de puntos, ciclistas, vehículos de emergencia y Move Over Law. No bloquea tu avance y puedes repetirla cuantas veces quieras.</p>
        </div>
      ...
      <form class="utah-v2-exam-form">
        <div class="utah-v2-exam-questions">
          ${questions.map((question,index) => `
            <fieldset class="utah-v2-exam-question" data-quiz-id="${esc(question.id)}">
              <legend><span class="utah-v2-exam-number">${index + 1}</span><span>${esc(question.question)}</span></legend>
              <div class="utah-v2-exam-options">
                ${(question.options || []).map(option => `
                  <label class="utah-v2-exam-option">
                    <input type="radio" name="module-q-${esc(question.id)}" value="${esc(option.key)}" required>
  ...
  function bind(card, exam) {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        const { data, error } = await db.rpc('submit_module_exam', { target_module: MODULE_ID, submitted_answers: answers });
        if (error) throw error;
        const percentage = Math.round(Number(data?.percentage || 0));
        resultHost.innerHTML = `<p class="utah-v2-exam-result ${percentage < 80 ? 'is-low' : ''}">${esc(data?.message || '')} Resultado: ${Number(data?.score || 0)}/${Number(data?.total || 0)} · ${percentage}%.</p>`;
        applyReview(form, data?.review || []);
        markSidebar(percentage);
  ...
  async function loadExam(card) {
    ...
    try {
      ...
      card.innerHTML = markup(data);
      markSidebar(data.best_percentage);
      bind(card, data);
      maybeScroll(card);
    } catch (error) {
      card.innerHTML = `<p class="utah-v2-exam-error">No se pudo cargar la evaluación. ${esc(error?.message || '')}</p>`;
    } finally {
  ...
  function ensureCard() {
    ...
    if (!context || context.lesson.lesson_code !== FINAL_CODE) return;
    ...
    card.setAttribute('aria-label','Evaluación opcional del Módulo 6');
    host.appendChild(card);
    loadExam(card);
  }

  function maybeScroll(card = document.querySelector(`.utah-v2-exam-card[data-module-id="${MODULE_ID}"]`)) {
    if (!card) return;
  ...
  window.ACADEMIA_AG_UTAH_MODULE6_EXAM_V2 = { release: RELEASE, enhance };
  schedule();
})();
