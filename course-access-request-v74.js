(() => {
  ...
  function pendingRequest(program, requests) {
    const subject = `Solicitud de acceso · ${program.title}`;
    return requests.find(item => item.subject === subject && ['open','in_progress'].includes(item.status));
  }

  function injectStyles() {
    if (document.querySelector('#course-access-request-v74-style')) return;
    const style = document.createElement('style');
    style.id = 'course-access-request-v74-style';
    style.textContent = `
      .student-catalog-banner-v74{margin:0 auto 28px;max-width:1180px;padding:18px 20px;border:1px solid rgba(0,81,52,.18);border-radius:18px;background:#f1f7f4;color:#17382d;display:flex;align-items:center;justify-content:space-between;gap:18px}
      .student-catalog-banner-v74 strong{display:block;margin-bottom:3px;color:#005134}
      .student-catalog-banner-v74 p{margin:0;color:#5f6f68;font-size:.9rem}
      .catalog-card .access-state-v74{display:flex;align-items:center;gap:8px;margin-top:14px;font-size:.76rem;font-weight:800;color:#005134}
      .catalog-card .access-state-v74::before{content:'✓';width:22px;height:22px;border-radius:50%;display:grid;place-items:center;background:#e8f5ee}
      .catalog-card .request-state-v74{display:flex;align-items:center;gap:8px;margin-top:14px;font-size:.76rem;font-weight:800;color:#8a6a15}
      .catalog-card .request-state-v74::before{content:'◷';width:22px;height:22px;border-radius:50%;display:grid;place-items:center;background:#fff7dc}
      .catalog-card [data-course-access-request][disabled]{opacity:.65;cursor:not-allowed;transform:none}
      .student-return-v74{white-space:nowrap}
      @media(max-width:760px){.student-catalog-banner-v74{align-items:flex-start;flex-direction:column}.student-catalog-banner-v74 .btn{width:100%}}
    `;
  ...
  function cardMarkup(program, enrollments, requests) {
    ...
    if (hasAccess && program.id) {
      actions = `
        <div class="access-state-v74">Ya tienes acceso a este curso</div>
      ...
      actions = `
        <div class="request-state-v74">Solicitud enviada · pendiente de revisión</div>
        <div class="catalog-actions"><button class="btn btn-primary" type="button" disabled>Solicitud enviada</button><a class="btn btn-secondary" href="#help">Ver mis solicitudes</a></div>`;
    } else {
      const label = available ? 'Solicitar acceso' : 'Solicitar información';
      actions = `
        <div class="catalog-actions"><button class="btn btn-primary" type="button" data-course-access-request data-course-id="${esc(program.id)}" data-course-title="${esc(program.title)}" data-course-available="${available ? '1' : '0'}">${label}</button><a class="btn btn-secondary" href="#help">Tengo una duda</a></div>`;
    }

    return `
      <article class="catalog-card" data-student-course-category="${esc(program.category)}">
        <div class="catalog-card-media">
          <img src="${esc(program.image)}" alt="${esc(program.title)}" onerror="imageErrorFallback(event)">
          <span class="status-pill ${available ? 'available' : ''}">${esc(program.status)}</span>
        </div>
  ...
  async function createAccessRequest(button) {
    ...
    const subject = `Solicitud de acceso · ${title}`.slice(0, 140);
    ...
    try {
      ...
      if (existing?.length) {
        showToast('Ya tienes una solicitud pendiente para este curso.');
        schedule(true);
        return;
      }
      ...
      const message = [
        available ? 'El alumno solicita acceso a un curso disponible desde el catálogo de Academia AG.' : 'El alumno solicita información y acceso cuando el programa esté disponible.',
        `Curso: ${title}`,
      ...
      showToast(available ? 'Solicitud de acceso enviada. El equipo de AG la revisará.' : 'Solicitud enviada. Te contactaremos cuando tengamos información.', 'success');
      schedule(true);
    } catch (error) {
  ...
  async function renderStudentCatalog(force = false) {
    ...
    try {
      ...
      if (headingTitle) headingTitle.textContent = 'Explora los cursos de Academia AG';
      if (headingText) headingText.textContent = 'Revisa los programas disponibles y solicita acceso directamente desde tu cuenta. El equipo de AG podrá aprobar tu inscripción desde el panel administrativo.';

      if (section && !section.querySelector('.student-catalog-banner-v74')) {
        const banner = document.createElement('div');
        banner.className = 'student-catalog-banner-v74';
        banner.innerHTML = `<div><strong>Estás explorando cursos con tu cuenta de alumno</strong><p>Si un curso todavía no está asignado a tu cuenta, puedes enviar una solicitud sin salir de la academia.</p></div><a class="btn btn-secondary" href="#courses">Volver a Mis cursos</a>`;
        const filters = section.querySelector('.catalog-filters');
        (filters || grid).insertAdjacentElement('beforebegin', banner);
      }

      const header = document.querySelector('.public-header');
      if (header && !header.querySelector('.student-return-v74')) {
        const oldButton = header.querySelector('[data-public-login]');
        if (oldButton) oldButton.style.display = 'none';
        const back = document.createElement('a');
        back.className = 'btn btn-primary student-return-v74';
        back.href = '#home';
