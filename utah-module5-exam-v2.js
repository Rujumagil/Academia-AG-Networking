(() => {
  ...
  const RELEASE = '20260820.52';
  const COURSE_ID = '7c4d9f60-8b0a-4b7b-9f2c-2d5e1a8c4f01';
  const MODULE_ID = '7c4d9f60-1005-4b7b-9f2c-2d5e1a8c4001';
  const PROMO_CODE = 'C5-PROMO';
  const SCROLL_KEY = 'academia-ag:utah-v2:m5-exam-scroll';
  let timer = null;
  ...
  function moduleDetailsNode() {
    ...
    return details.find(node => /^Módulo\s+5\b/i.test(node.querySelector('summary strong')?.textContent?.trim() || '')) || null;
  }
  ...
  function ensureSidebarStep() {
    ...
    if (!promo || !list || list.querySelector('.utah-v2-exam-row-m5')) return;

    const row = document.createElement('div');
    row.className = 'lesson-item utah-v2-exam-row utah-v2-exam-row-m5';
    row.innerHTML = `
      <span class="utah-v2-exam-dot" aria-hidden="true">?</span>
      <a href="#lesson/${COURSE_ID}/${esc(promo.id)}" data-utah-v2-m5-exam-link>
        <strong>Evaluación opcional</strong>
        <small>Repaso del Módulo 5 · 10 preguntas</small>
      </a>
      <small>Opcional</small>`;

    row.querySelector('[data-utah-v2-m5-exam-link]')?.addEventListener('click', () => {
  ...
  function markSidebarExam(bestPercentage = 0) {
    const row = moduleDetailsNode()?.querySelector('.utah-v2-exam-row-m5');
    if (!row) return;
  ...
  function examMarkup(exam) {
    ...
    return `
      <div class="utah-v2-exam-top">
        <div>
          <span class="utah-v2-exam-kicker">Evaluación opcional · Módulo 5</span>
          <h2>Alcohol, drogas y retos al manejar</h2>
          <p class="utah-v2-exam-intro">Repasa deterioro por alcohol y drogas, BAC de Utah, Not-a-Drop, emociones, celular, fatiga, zonas de construcción y derrapes. No bloquea tu avance y puedes repetirlo cuantas veces quieras.</p>
        </div>
  ...
  function ensureExamCard() {
    ...
    card.setAttribute('aria-label', 'Evaluación opcional del Módulo 5');
  ...
  window.ACADEMIA_AG_UTAH_MODULE5_EXAM_V2 = { release: RELEASE, enhance };
  schedule();
})();
