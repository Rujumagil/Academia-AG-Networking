...
# Version 2: corrige referencias de variables seguidas por ':' en PowerShell.
...
function Invoke-CfGetVideo([string]$Uid, $Headers) {
...
function Upload-Video([string]$Path, [string]$Code, [string]$Token) {
    ...
    Write-Host ('    Archivo: {0} ({1} MB)' -f [IO.Path]::GetFileName($Path), $sizeMB) -ForegroundColor DarkGray

    if ($size -le 0) {
        throw ('{0}: el archivo seleccionado esta vacio.' -f $Code)
    }
    if ($size -ge 200MB) {
        throw ('{0}: el archivo mide {1} MB. Esta herramienta usa subida simple y requiere un archivo menor de 200 MB.' -f $Code, $sizeMB)
    }
    ...
    try {
        ...
        $uploadName = '{0}{1}' -f $Code, [IO.Path]::GetExtension($Path).ToLowerInvariant()
        ...
        if (-not $httpResponse.IsSuccessStatusCode) {
            throw ('{0}: Cloudflare rechazo la subida directa. HTTP {1}. {2}' -f $Code, [int]$httpResponse.StatusCode, $responseText)
        }
...
try {
    ...
    foreach ($item in $Items) {
        if ($Checkpoint.ContainsKey($item.Code) -and $Checkpoint[$item.Code].uid) {
            ...
            try {
                ...
                if ($existing.success) {
                    Write-Host ('    {0} ya tiene UID {1}. Se omite nueva subida.' -f $item.Code, $existingUid) -ForegroundColor DarkGray
                    continue
                }
            }
            catch { }
        ...
        Write-Host ''
        Write-Host ('    {0} - {1}' -f $item.Code, $item.Title) -ForegroundColor White
        ...
        if ([string]::IsNullOrWhiteSpace($selected)) {
            throw ('{0}: no se selecciono ningun archivo. Puedes volver a ejecutar la herramienta sin perder avances.' -f $item.Code)
        }

        Write-Host ('    Subiendo {0}...' -f $item.Code) -ForegroundColor Cyan
        ...
        if (-not $response.success -or -not $response.result.uid) {
            ...
            throw ('Cloudflare no devolvio UID para {0}. {1}' -f $item.Code, $message)
        }
        ...
        Write-Host ('    UID recibido: {0}' -f $uid) -ForegroundColor Green
    }
    ...
    foreach ($item in $Items) {
        if (-not $Checkpoint.ContainsKey($item.Code) -or -not $Checkpoint[$item.Code].uid) {
            throw ('Falta UID para {0}.' -f $item.Code)
        }
        ...
        while ($true) {
            ...
            if (-not $detail.success) {
                throw ('No se pudo consultar {0} ({1}).' -f $item.Code, $uid)
            }
            ...
            if ($state -eq 'error') {
                throw ('Cloudflare marco ERROR en {0}: {1}' -f $item.Code, $v.status.errorReasonText)
            }
    ...
    $sql.Add('-- Generado por migrar-intro-promo-cloudflare-v2.ps1')
    ...
    Write-Host ('    SQL FINAL: {0}' -f $SqlPath) -ForegroundColor Cyan
    ...
    Write-Host ''
    Write-Host ('No borres {0}. Vuelve a ejecutar el lanzador para continuar sin duplicar lo ya subido.' -f $CheckpointPath) -ForegroundColor Yellow
    exit 1
}
finally {
    $Token = $null
